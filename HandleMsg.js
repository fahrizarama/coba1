    ...
    try {
        ...
        switch (command) {
            ...
            case 'stiker':
                if ((isMedia || isQuotedImage) && args.length === 0) {
                    ...
                    pakforlay.sendImageAsSticker(from, imageBase64)
                        .then(() => {
                            pakforlay.reply(from, 'Taraa... Stickermu sudah jadi')
                            console.log(`Sabar yo.... ${processTime(t, moment())} Second`)
                        })
                } else if (args[0] === 'nobg') {
                    if (isMedia || isQuotedImage) {
                        try {
                            ...
                            console.log(err)
                            await pakforlay.reply(from, 'Maaf ya masih Beta..', id)
                        }
                    }
                    ...
                    pakforlay.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                        ...
                        : pakforlay.reply(from, 'Yeyy.. Stickermu Jadii')).then(() => console.log(`Sabar yo.. Stickermu sedang dibuatkan ${processTime(t, moment())} Second`))
                } else {
            ...
            case 'stikergif':
                if (isMedia || isQuotedVideo) {
                    if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                        var mediaData = await decryptMedia(message, uaOverride)
                        pakforlay.reply(from, 'Sabar yoo.. Stikermu sedang dibuatkan..', id)
            ...
            case 'meme':
                if ((isMedia || isQuotedImage) && args.length >= 2) {
                    ...
                    pakforlay.sendFile(from, ImageBase64, 'image.png', '', null, true)
                        .then(() => {
                            pakforlay.reply(from, 'awokowk', id)
                        })
                        .catch(() => {
                            pakforlay.reply(from, 'Error, Hmm mungkin gambar terlalu besar')
                        })
                } else {
                    await pakforlay.reply(from, `Hmm gambarnya mana! Kirim gambar dengan caption ${prefix}meme <teks_atas> | <teks_bawah>\ncontoh: ${prefix}meme teks atas | teks bawah`, id)
                }
                break
            case 'quotemaker':
                ...
                if (qmaker.length >= 3) {
                    ...
                    pakforlay.reply(from, 'Sabar, Sedang proses..', id)
                    try {
                        const hasilqmaker = await images.quote(quotes, author, theme)
                        pakforlay.sendFileFromUrl(from, `${hasilqmaker}`, '', 'Ini dia..', id)
                    } catch {
                        pakforlay.reply('Proses gagal, Cek lagi isinya dengan benar ya..', id)
                    }
                } else {
                    pakforlay.reply(from, `Pemakaian ${prefix}quotemaker |isi quote|author|theme\n\ncontoh: ${prefix}quotemaker |aku sayang kamu|-Hasemeleh|random\n\nuntuk theme nya pakai random ya kak..`)
                }
                break
            case 'nulis':
                if (args.length == 0) return pakforlay.reply(from, `Membuat bot menulis teks yang dikirim menjadi gambar\nPemakaian: ${prefix}nulis [teks]\n\ncontoh: ${prefix}nulis i love you`, id)
                const nulisq = body.slice(7)
                const nulisp = await rugaapi.tulis(nulisq)
                await pakforlay.sendImage(from, `${nulisp}`, '', 'Nih, Ga perlu capek-capek nulis', id)
                    .catch(() => {
                        pakforlay.reply(from, 'Ada yang Error!', id)
                    })
                break
            ...
            case 'instagram':
                ...
                break
            case 'tiktok':
                if (args.length == 0) return pakforlay.reply(from, `Untuk mendownload video tiktok\nketik: ${prefix}tiktok https://www.tiktok.com/@vinnyvirgio/video/6893872754173480194`, id)
            ...
            case 'tts':
                ...
                if (dataText === '') return pakforlay.reply(from, 'apa teksnya anjim..', id)
                try {
